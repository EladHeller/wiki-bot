...
function textFromMatch(
  ...
  const link = url.startsWith(HAMICHLOL_DOMAIN)
    ? `[${url} ${decodeURIComponent(url.replace(HAMICHLOL_DOMAIN, '')
      .replace(/_/g, ' '))} במכלול]`
...
export async function runCopyrightViolationBot(basePage: string, runMode: 'new' | 'edit') {
  const LAST_RUN_PAGE = `${basePage}/ריצה אחרונה`;
  const TEMP_ERRORS_PAGE = `${basePage}/שגיאות זמניות`;
  const LOG_PAGE = `${basePage}/לוג`;

  const api = WikiApi();
  await api.login();
  const currentRun = new Date();
  const lastRun = await getLastRun(api, LAST_RUN_PAGE);
  ...
  const generator = api.recentChanges([0, 2, 118], lastRun.content, 500, runMode);
  ...
  await writeAdminBotLogs(api, allLogs, basePage, false);
