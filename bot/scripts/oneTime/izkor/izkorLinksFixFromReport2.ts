...
const FIX_RESULTS_PAGE_TITLE = 'משתמש:Sapper-bot/ניסיונות תיקון קישורי יזכור';
  ...
  try {
    ...
    if (!match) {
      return {
        ...
        attemptedUrl: finalUrl,
        success: false,
        error: 'Could not extract final ID from URL',
      };
    }
    const fixedName = decodeURIComponent(match[1]).replaceAll(/\s[א-ת]+\s/g, ' ');
    const fixedUrl = finalUrl.replace(match[1], encodeURIComponent(fixedName));
...
async function createFixReport(api: IWikiApi, results: FixAttemptResult[]): Promise<void> {
  ...
  try {
    ...
    await api.edit(FIX_RESULTS_PAGE_TITLE, 'דוח ניסיות תיקון קישורי יזכור', reportContent, revid);
